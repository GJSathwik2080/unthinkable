    ...
    try {
      ...
      const payload = {
        ...
        pickup: {
          ...
          phone: normalizePhone(form.get("pickupPhone")),
          addressLine1: String(form.get("pickupAddress") ?? ""),
          postalCode: input.pickupPostalCode,
        },
        drop: {
          recipientName: String(form.get("dropRecipientName") ?? ""),
          phone: normalizePhone(form.get("dropPhone")),
          addressLine1: String(form.get("dropAddress") ?? ""),
...
function Profile({ role }: { role: UserRole }) { const user = useRemote<CurrentUser>("/api/auth/me"); const dashboard = useRemote<Dashboard>(role === "DELIVERY_AGENT" ? "/api/dashboard" : null); const [error, setError] = useState(""); const changeAvailability = async () => { const availability = window.prompt("Availability: AVAILABLE, BUSY, or OFFLINE"); if (!availability) return; try { await api("/api/agent/availability", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ availability }) }); dashboard.reload(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to update availability."); } }; return <><PageTitle eyebrow="Account" title="Profile" /><section className="profile-grid"><article className="panel"><State loading={user.loading} error={user.error} empty={!user.data}>{user.data && <><p className="section-kicker">Personal details</p><h2>{user.data.fullName}</h2><div className="profile-details"><span><small>Email</small><b>{user.data.email}</b></span><span><small>Role</small><b>{roleLabels[user.data.role]}</b></span></div></>}</State></article>{role === "DELIVERY_AGENT" && <article className="panel"><p className="section-kicker">Work status</p><h2>{dashboard.data?.agent?.availability ?? "Loading…"}</h2><p className="form-error">{error}</p><button className="button secondary" onClick={() => void changeAvailability()}>Change availability</button></article>}</section></>;
}
