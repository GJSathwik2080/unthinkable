...
const schema = z.object({ fullName: z.string().trim().min(2).max(120), phone: z.string().trim().min(8).max(24).optional(), email: z.email(), password: z.string().min(8).max(128) });
...
export async function POST(request: NextRequest) {
  ...
  const response = NextResponse.json({ success: true, data: { message: "Registration successful. Check your email if confirmation is enabled." } }, { status: 201 });
  try {
